...
$(function(){
   ...
   $('.reply').each(function(index,item){  //답변 버튼을 누를때
      $(this).on('click',function(){
         $('.fcomm_re_ct_area').hide();
         $(this).parents('li').find('.fcomm_re_ct_area').show();
      })
   })
   ...
   <ul>
      <c:forEach var="comm" items="${bean}">
         ...
         <input type="hidden" class="fcomm_no" name="fcomm_no"
            ...
            <c:set var="num" value="${comm.fcomm_re_lev}" />
            ...
            <div class="fcomm_re_ct_area comm_re_ct_area">
               ...
               <button class="fcomm_rep_button comm_rep_button" >댓글남기기</button>
            </div>
         </li>
      </c:forEach>
   </ul>
...
<div class="fcomm_ct_area comm_ct_area">
   <textarea cols="70" rows="20" id="fcomm_ct"></textarea>
   <button onclick="fcomm_write()" class="fcomm_rep_buttonL comm_rep_buttonL">코멘트남기기</button>
</div>
</c:if>