		...
		<div class="subContent"> 
			...
			<div class="photolist">
				...
				<ul>
					...
					<c:forEach var="b" items="${gbbslist}">
			...
			<div class="paginate">
				<p>
					...
					<c:if test="${page > 1 }">
						<a href="gbbs_find_ok.brn?page=${page-1}&limit=${limit}&gfind_field=${gfind_field}&gfind_name=${gfind_name}">&lt;</a>
					</c:if>			
					...
					<c:forEach var="a" begin="${startpage}" end="${endpage}">
						...
						<c:if test="${a != page }">
							<a href="gbbs_find_ok.brn?page=${a}&limit=${limit}&gfind_field=${gfind_field}&gfind_name=${gfind_name}">${a}</a>
						</c:if>
					</c:forEach>			
				...
				<script>
				   function find_check(){
					   if($.trim($("#gfind_name").val())==""){
						   alert("검색이름을 입력하세요!");
						   $("#gfind_name").val("").focus();
						   return false;
					   }
				   }
				</script>
